...
@section('content')
    <div class="max-w-3xl mx-auto">
        <div class="bg-white/70 backdrop-blur-xl rounded-3xl p-8 border border-white/50 shadow-xl shadow-gray-100/50">
            <div class="flex items-center gap-3 mb-6">
                <div
                    class="w-10 h-10 bg-gradient-to-br from-indigo-100 to-purple-100 rounded-2xl flex items-center justify-center">
            ...
            <form action="{{ route('appointments.store') }}" method="POST" id="appointmentForm">
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        <label for="client_search" class="block text-sm font-semibold text-gray-700 mb-1.5">Клиент
                            *</label>
                        <div class="relative">
                            <div class="flex gap-2">
                                <div class="flex-1 relative">
                                    ...
                                    <div id="clientResults"
                                         class="hidden absolute z-50 w-full mt-1 bg-white/95 backdrop-blur-xl rounded-2xl shadow-2xl border border-gray-100/50 max-h-60 overflow-y-auto">
                                        <div id="clientResultsList"></div>
                                        <div id="clientCreateNew" class="hidden p-3 border-t border-gray-100">
                                            <button type="button" id="createNewClientBtn"
                                                    class="w-full flex items-center gap-2 px-4 py-2 bg-indigo-50 text-indigo-700 rounded-xl hover:bg-indigo-100 transition">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor"
                                                     viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round"
                                                          stroke-width="2" d="M12 4v16m8-8H4"/>
                                                </svg>
                                                <span>Создать нового клиента: <span id="newClientName"></span></span>
                                            </button>
                                        </div>
                                        <div id="clientLoading" class="hidden p-4 text-center text-gray-400">
                                            <svg class="w-5 h-5 animate-spin mx-auto" fill="none" stroke="currentColor"
                                                 viewBox="0 0 24 24">
                                        ...
                                        <div id="clientNoResults" class="hidden p-4 text-center text-gray-400">
                                            ...
                                            <button type="button" id="createNewClientBtnNoResults"
                                                    class="mt-2 inline-flex items-center gap-2 px-4 py-2 bg-indigo-600 text-white rounded-xl text-sm hover:bg-indigo-700 transition">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor"
                                                     viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round"
                                                          stroke-width="2" d="M12 4v16m8-8H4"/>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            <label for="employee_id" class="block text-sm font-semibold text-gray-700 mb-1.5">Сотрудник
                                *</label>
                        ...
                        <div>
                            <label for="service_id" class="block text-sm font-semibold text-gray-700 mb-1.5">Услуга
                                *</label>
                            <select id="service_id" name="service_id" required
                                ...
                                @foreach($services as $service)
                                    <option value="{{ $service->id }}"
                                        ...
                                        мин, {{ number_format($service->price, 0, '.', ' ') }} ₽)
                                    </option>
                                @endforeach
                            </select>
                            @error('service_id')
    ...
    <div id="createClientModal"
        ...
        <div class="bg-white/95 backdrop-blur-xl rounded-3xl max-w-md w-full p-8 shadow-2xl border border-white/50">
            ...
            <form id="createClientForm">
                ...
                <div class="space-y-4">
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-1.5">Имя *</label>
                        <input type="text" id="modal_first_name" name="first_name" required
                               class="w-full px-4 py-3 bg-gray-50/80 border border-gray-200/80 rounded-2xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 focus:bg-white transition outline-none"
                               placeholder="Иван">
                    </div>
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-1.5">Фамилия</label>
                        <input type="text" id="modal_last_name" name="last_name"
                               class="w-full px-4 py-3 bg-gray-50/80 border border-gray-200/80 rounded-2xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 focus:bg-white transition outline-none"
                               placeholder="Иванов">
                    </div>
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-1.5">Телефон *</label>
                    ...
                    <div id="modalError" class="hidden text-rose-600 text-sm"></div>
                </div>

                <div class="flex items-center justify-end gap-3 mt-6 pt-6 border-t border-gray-100/80">
                    <button type="button" id="cancelClientModal"
                            class="px-6 py-2.5 rounded-xl text-sm font-medium text-gray-600 hover:bg-gray-100/80 transition">
                        Отмена
                    </button>
                    <button type="submit" id="saveClientBtn"
                            class="px-6 py-2.5 bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-2xl font-medium shadow-lg shadow-indigo-500/25 hover:shadow-xl hover:shadow-indigo-500/30 transition-all hover:scale-[1.02]">
                        Создать
                    </button>
                </div>
            </form>
    ...
    @push('scripts')
        <script>
            document.addEventListener('DOMContentLoaded', function () {
                    ...
                    fetch(`/api/clients/search?q=${encodeURIComponent(query)}`)
                        ...
                        .then(data => {
                            ...
                            if (data.length === 0) {
                                ...
                                newClientName.textContent = query;
                                return;
                            }
                ...
                function openModal() {
                    ...
                    if (searchInput.value.trim().length > 0) {
                        document.getElementById('modal_first_name').value = searchInput.value.trim();
                    }
                ...
                function closeModal() {
                    ...
                    saveClientBtn.textContent = 'Создать';
                }
                ...
                createClientForm.addEventListener('submit', function(e) {
                    ...
                    fetch('/api/clients', {
                        ...
                        .then(result => {
                            ...
                            if (result.success === true || result.success === 'true' || result.data || result.id) {
            ...
            `;
                                ...
                                modalError.textContent = result.message || 'Ошибка создания клиента';
                        ...
                        .catch(error => {
                            ...
                            saveClientBtn.disabled = false;
                            saveClientBtn.textContent = 'Создать';
                            modalError.textContent = 'Ошибка сети. Попробуйте еще раз.';
                            modalError.classList.remove('hidden');
                        });
                });
            ...
            `;
                ...
                clientIdInput.addEventListener('change', checkAvailability);

                // ============================================
                // ДОПОЛНИТЕЛЬНО: ПРОВЕРКА СЛОТОВ ПРИ ИЗМЕНЕНИИ ДАТЫ/ВРЕМЕНИ
                // ============================================
                // Принудительная проверка после загрузки, если есть выбранные значения
                if (employeeSelect.value && dateInput.value && serviceSelect.value && clientIdInput.value) {
                    checkAvailability();
                }
                ...
                // Очистка индикатора ошибки при вводе
                searchInput.addEventListener('input', function () {
